...
namespace http_handler {
    ...
    StringResponse APIHandler::ReturnAPIResponse(const StringRequest&& req, std::string req_str) {
        ...
        /* ----------------------------------- запрашивается список игроков в сессии ----------------------------------- */
            ...
            if ((req.method() == http::verb::get) || (req.method() == http::verb::head)) {
                return ExecuteAuthorized(req, [this](players::Player* found_player,
        ...
        /* ----------------------------------- запрос на управление персонажем ----------------------------------- */
            ...
            if (req.method() == http::verb::post) {
                ...
                return ExecuteAuthorized(req, [this, &req](players::Player* found_player,
        ...
        } else if (req_str.starts_with(command_tick_str)) {
            if (req.method() == http::verb::post) {
                if (ContentType::JSON.compare(req[http::field::content_type]) != 0) {
                    return text_response(http::status::bad_request, json_loader::MakeErrorString("invalidArgument",
                                                                                                 "Invalid content type"));
     ...
     */
    StringResponse APIHandler::HandlePlayersList(players::Player* found_player,
                                                 unsigned int version,
                                                 bool keep_alive,
                                                 bool head_only) {
        const auto text_response = [version, keep_alive](http::status status, std::string_view text, size_t length = 0,
     ...
     */
    StringResponse APIHandler::HandleGameState(players::Player* found_player,
                                               unsigned int version,
                                               bool keep_alive,
                                               bool head_only) {
        const auto text_response = [version, keep_alive](http::status status, std::string_view text, size_t length = 0,
        ...
        for (const players::Player *next_player : app_.GetPlayersInSession(found_player)) {
    ...
    StringResponse APIHandler::HandleAction(players::Player *found_player,
                                            std::string_view body,
                                            unsigned int version,
                                            bool keep_alive) {
        const auto text_response = [version, keep_alive](http::status status, std::string_view text, size_t length = 0,
