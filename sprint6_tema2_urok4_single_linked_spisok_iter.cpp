 ...
 * 1) конструктор по умолчанию, который создаёт пустой список;
 * 2) метод GetSize, который возвращает количество элементов в списке;
 * 3) метод IsEmpty, который возвращает true, если список пустой, и false в противном случае.
 * 4) метод PushFront делает вставки элемента в начало односвязного списка,
 * должен предоставлять строгую гарантию безопасности исключений: если в процессе работы метода будет выброшено исключение,
 * состояние списка должно быть таким же, как до вызова метода.
 * 5) Метод Clear очищает список и не должен выбрасывать исключений. Считается, что деструктор типа Type не выбрасывает исключений.
 ...
 * 6) Реализуйте шаблонный класс BasicIterator, на основе которого будут объявлены константный и неконстантный итераторы списка.
 * 7) В классе списка реализуйте константную и неконстантную версии методов begin и end, которые возвращают итераторы на
 * первый элемент контейнера и позицию, следующую за последним элементом.
...
class SingleLinkedList {
    ...
    public:
        ...
        BasicIterator operator++(int) noexcept {
            Node* temp_node = node_;
            node_ = node_->next_node;
            return BasicIterator(temp_node);
        }
...
public:
    ...
    [[nodiscard]] Iterator end() noexcept {
        ...
        return BasicIterator<Type>(tmp_node->next_node);*/
        Node* result = nullptr;
        return BasicIterator<Type>(result);
    }
    ...
    [[nodiscard]] ConstIterator begin() const noexcept {
        const BasicIterator<Type> iter(head_.next_node);
        return iter;
    }
    ...
    [[nodiscard]] ConstIterator end() const noexcept {
        Node* result = nullptr;
        const BasicIterator<Type> iter(result);
        return iter;
    }
